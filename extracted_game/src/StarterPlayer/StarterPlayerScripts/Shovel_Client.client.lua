...
local VALUABLE_PLANTS = {
	"Purple Cabbage", "Coconut", "Cactus", "Eggplant", "Dragon Fruit", 
	"Mango", "Grape", "Mushroom", "Peach", "Raspberry", "Pineapple", 
	"Papaya", "Banana", "Passionfruit", "Soul Fruit", "Cursed Fruit", 
	"Cranberry", "Durian", "Lotus", "Venus Fly Trap", "Candy Blossom", 
	"Cherry Blossom", "Lemon", "Pear"
}
...
ConfirmButton.MouseButton1Click:Connect(function()
	if targetPlant then
		RemoveItemEvent:FireServer(targetPlant)
		targetPlant = nil
	end
...
local function handleShovelInput(position, isTouch)
	if isTouch or not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("Shovel [Destroy Plants]") then
		return
	end
	...
	if isRestrictedPlant(plantName) then
		Notification:CreateNotification(`You cannot destroy {plantName}!`)
		return
	end
	...
	if isValuablePlant(plantName) then
		...
		RemoveItemEvent:FireServer(raycastResult.Instance)
	end
end
